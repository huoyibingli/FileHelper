...
namespace Eca.File
{
    public class FtpHelper
    {
        ...
        public bool UploadFtp(string fileName, Stream fileStream)
        {
            CreateFtpDirectory();
            byte[] filebytes = fileStream.ExToBytes();
            bool result = false;
        ...
        private bool CheckIfDirectoryExists(string ftpPathDir)
        {
            ...
            {
                ServiceLocator.EcaLog.Error("FTP目录不存在！", ftpPathDir + ex.Message);
                req.Abort();
                return false;
            }
            req.Abort();
        ...
        private bool MakeDirectory(string ftpPathDir)
        {
            ...
            {
                ServiceLocator.EcaLog.Error("创建FTP目录：", ftpPathDir + ex.Message);
                req.Abort();
                return false;
            }
            req.Abort();
